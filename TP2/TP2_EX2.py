...
# {DPID:{PORT: IP}}
interface_port_to_ip = {1: {1: '192.168.10.254', 2: '192.168.20.254', 3: '192.168.30.254'}}
...
class L3Switch(app_manager.RyuApp):
	...
	#descartar todos os pacotes ipv6
	...
	#the controller uses this message to query of all the ports
    ...
    def port_desc_stats_reply_handler(self, ev):
        ...
        for p in ev.msg.body:
            ...
            if not p.port_no == 4294967294:   #porta de controlo entre o controlador e o switch
	...
	#se o pacote for ARP
		...
		#se o ip ja estiver na tabela adiciona na tabela flow nos 2 sentidos e forward pacote com mac atualizado
            if dst_ip in self.ip_to_mac[dpid]:
                self.logger.info('NEW FLOW ADDED, CHECK FLOW TABLE')
            	...
            	#se for dentro do mesmo switch L3
    ...
    def handle_arp(self, msg, pkt, in_port, src_mac):
        ...
        if arp_pkt.opcode == arp.ARP_REQUEST: #ARP REQ
            self.update_mac_table(datapath, src_mac, in_port)
            self.update_ip_table(datapath, src_ip, src_mac)
        ...
        elif arp_pkt.opcode == arp.ARP_REPLY:   # ARP REPLY
            if not self.queue[dpid]: #If QUEUE is empty
                self.logger.info('WARNING! -> Queue is empty!')
                return
                ...
                self.logger.info('WARNING! -> ARP REPLY not REQUESTED!')
                return
        else:
            return        
	#Forward packets in queue
	...
	#Forward packets if not in queue
	...
	#injetar flow na tabela de flows
    ...
    def firewall(self, datapath):
        ...
        match = datapath.ofproto_parser.OFPMatch(in_port=1, eth_type = ether_types.ETH_TYPE_IP, ip_proto=6, ipv4_src= ('192.168.30.3', '255.255.255.255'), ipv4_dst = ('192.168.10.3', '255.255.255.255'), tcp_src = tcp_port)
        actions = [datapath.ofproto_parser.OFPActionSetField(eth_src = '30:00:00:00:00:03'), datapath.ofproto_parser.OFPActionSetField(eth_dst = '10:00:00:00:00:03'), datapath.ofproto_parser.OFPActionOutput(3)]
        self.add_flow(datapath, priority1, match, actions)
        
        match = datapath.ofproto_parser.OFPMatch(in_port=1, eth_type = ether_types.ETH_TYPE_IP, ip_proto=6, ipv4_src= ('192.168.10.3', '255.255.255.255'), ipv4_dst = ('192.168.30.3', '255.255.255.255'))
        actions = [datapath.ofproto_parser.OFPActionSetField(eth_src = '10:00:00:00:00:03'), datapath.ofproto_parser.OFPActionSetField(eth_dst = '30:00:00:00:00:03'), datapath.ofproto_parser.OFPActionOutput(3)]
        self.add_flow(datapath, priority1, match, actions)

        # Allow TCP from HTTP server (H3) to H5 and vice-versa in specific port!
        match = datapath.ofproto_parser.OFPMatch(in_port=1, eth_type = ether_types.ETH_TYPE_IP, ip_proto=6, ipv4_src= ('192.168.20.2', '255.255.255.255'), ipv4_dst = ('192.168.10.3', '255.255.255.255'), tcp_src = tcp_port)
        actions = [datapath.ofproto_parser.OFPActionSetField(eth_src = '20:00:00:00:00:02'), datapath.ofproto_parser.OFPActionSetField(eth_dst = '10:00:00:00:00:03'), datapath.ofproto_parser.OFPActionOutput(2)]
        self.add_flow(datapath, priority1, match, actions)
        
        match = datapath.ofproto_parser.OFPMatch(in_port = 1, eth_type = ether_types.ETH_TYPE_IP, ip_proto=6, ipv4_src= ('192.168.10.3', '255.255.255.255'), ipv4_dst = ('192.168.20.2', '255.255.255.255'))
        actions = [datapath.ofproto_parser.OFPActionSetField(eth_src = '10:00:00:00:00:03'), datapath.ofproto_parser.OFPActionSetField(eth_dst = '20:00:00:00:00:02'), datapath.ofproto_parser.OFPActionOutput(2)]
        self.add_flow(datapath, priority1, match, actions)
        
        # Block packets between networks 
        match = datapath.ofproto_parser.OFPMatch(eth_type = ether_types.ETH_TYPE_IP, ipv4_src= ('192.168.10.0', '255.255.255.0'))
        actions = []
        self.add_flow(datapath, priority2, match, actions)

        match = datapath.ofproto_parser.OFPMatch(eth_type = ether_types.ETH_TYPE_IP, ipv4_src= ('192.168.20.0', '255.255.255.0'))
        actions = []
        self.add_flow(datapath, priority2, match, actions)

        match = datapath.ofproto_parser.OFPMatch(eth_type = ether_types.ETH_TYPE_IP, ipv4_src= ('192.168.30.0', '255.255.255.0'))
        actions = []
        self.add_flow(datapath, priority2, match, actions)
